<script lang="ts">
    ...
    function scaled(d: Drag, u: number, v: number, p: Point, e: PointerEvent) {
        ...
        const enforceAR =
            (transformModel === "scale" || transformModel === "scale+rotate") &&
            keepAspectRatio;
        ...
        switch (d.op.kind) {
            case "move":
                onChange(
                    inScreen(
                        d,
                        translationMatrix(p.x - d.start.x, p.y - d.start.y),
                    ),
                );
                break;
            case "rotate": {
                ...
                onChange(
                    inScreen(d, aroundPoint(rotationMatrix(angle), c.x, c.y)),
                );
                break;
            }
            case "pivot": {
