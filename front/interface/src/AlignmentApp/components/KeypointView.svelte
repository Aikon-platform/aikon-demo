<script lang="ts">
  ...
  const view = $derived.by(() => {
    const s =
      Math.min(containerWidth / width, containerHeight / height) * FIT_MARGIN;
  ...
  function clearHover() {
    if (alignmentState.hover?.image === imageIndex) alignmentState.hover = null;
  }
  ...
  function handlePointerMove(e: PointerEvent) {
    ...
    if (drag.kind === "pan") {
      ...
      const p = toImage(screen);
      if (p) alignmentState.moveKeypoint(imageIndex, drag.k, clampToImage(p));
    }
  }
  ...
  const hoverScreen = $derived.by(() => {
    ...
    return toScreen(alignmentState.warpPoint(hover.image, imageIndex, hover.point));
  });
